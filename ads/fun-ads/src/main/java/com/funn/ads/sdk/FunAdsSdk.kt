...
import com.funn.ads.domain.model.FilteredPreloads
...
object FunAdsSdk {
    ...
    private var registeredPreloads = HashMap<String, FilteredPreloads>()
    ...
    fun getPreloadByPlacementKey(key: String) = registeredPreloads[key]
    ...
    private fun registerPlacements(): HashMap<String, UiAdFiltered> {
        ...
        adsData?.uiAds?.let {
            ...
            it.registered?.forEach { registeredAd ->
                if (registeredAd.placementKey.isNotBlank() && registeredAd.enabled.toBoolean() && registeredAd.adName.isNotBlank()) {
            ...
            when (model.adType) {
                ...
                AdType.REWARDED -> AdmobRewardedAdsManager.addNewController(
                    AdmobRewardedAdsController(
                        adKey = model.adKey, adIdsList = model.adIds, listener = listener
                    ), true
                )
