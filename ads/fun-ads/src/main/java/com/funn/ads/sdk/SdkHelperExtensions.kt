...
fun AdsJsonModel?.toFilteredPreloads(
    controllers: List<ControllerModel>
): HashMap<String, FilteredPreloads> {
    val filteredPreloads = HashMap<String, FilteredPreloads>()
    this?.preloadRequests?.list?.forEach { request ->
        if (request.enabled.toBoolean()) {
            val adsToLoad = mutableListOf<AdKeyWithType>()
            request.loadAds.forEach { adKey ->
                val controller: ControllerModel? = controllers.firstOrNull { cont ->
                    cont.adKey == adKey
                }
                if (controller != null) {
                    adsToLoad.add(
                        AdKeyWithType(
                            adKey = controller.adKey,
                            adType = controller.adType
                        )
                    )
                }
            }
            filteredPreloads[request.placementKey] = FilteredPreloads(
